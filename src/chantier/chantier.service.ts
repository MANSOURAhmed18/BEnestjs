  ...
  async assignPersonnel(
    ...
    chantier.personnelAssignments = [
      ...(chantier.personnelAssignments || []).filter(
        (pa) =>
          !(
            pa.personnel.toString() === personnelId &&
            pa.date.toISOString() === date.toISOString()
          ),
      ),
  ...
  async assignVehicule(
    ...
    chantier.vehiculeAssignments = [
      ...(chantier.vehiculeAssignments || []).filter(
        (va) =>
          !(
            va.vehicule.toString() === vehiculeId &&
            va.date.toISOString() === date.toISOString()
          ),
      ),
